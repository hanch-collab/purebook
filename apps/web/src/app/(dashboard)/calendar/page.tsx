...
function ApptCard({ appt, svc }: { appt: Appointment; svc: AppointmentService }) {
  ...
  return (
    <div className="absolute left-1 right-1 rounded-md px-2 py-1.5 cursor-pointer overflow-hidden z-[3] border-l-[3px]"
      style={{ top, height: ht, background: bg, borderLeftColor: bd }}>
      <div className="text-[10px] font-medium truncate" style={{ color: bd }}>{svc.service.name}</div>
      {ht > 32 && (
        <div className="flex items-center gap-1 mt-0.5">
          <span className="text-[10px] flex-1 truncate opacity-85" style={{ color: bd }}>
            {appt.customer.firstName} {appt.customer.lastName}
          </span>
          <span className="text-[8px] font-medium px-1 py-0.5 rounded whitespace-nowrap" style={{ background: badge.bg, color: badge.text }}>
            {badge.label}
          </span>
        </div>
      )}
...
export default function CalendarPage() {
  ...
  // Now-line — local time only, single calculation
  useEffect(() => {
    function update() {
      const now = new Date();
      const isToday = toYMD(now) === toYMD(date);
      if (!isToday) { setNowTop(null); return; }
      const h = now.getHours() + now.getMinutes() / 60;
      if (h < HOUR_START || h > HOUR_END) { setNowTop(null); return; }
      setNowTop((h - HOUR_START) * CELL_H);
      setNowLabel(now.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }));
    }
    update();
  ...
  const gridHeight = (HOUR_END - HOUR_START) * CELL_H;
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-pb-bg">
      ...
      <div className="h-[52px] px-4 flex items-center gap-2.5 border-b border-pb-border bg-pb-bg flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 ml-1.5">
          ...
          <div className="text-[12px] font-medium text-pb-text min-w-[148px] text-center px-2.5 py-1 rounded-md bg-[#EAE6E0]">{fmtDate(date)}</div>
          <button onClick={() => navDate(1)} className="w-[26px] h-[26px] rounded-md border border-pb-border bg-transparent text-pb-muted flex items-center justify-center text-sm">›</button>
        </div>
      ...
      <div className="h-9 px-4 flex items-center gap-2 border-b border-pb-border bg-[#EDE9E3] flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-[#4A4740]">
          <div className="w-8 h-[17px] rounded-full bg-[#DDD8D0] relative cursor-pointer">
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div style={{ minWidth: "max-content" }}>
              ...
              <div className="flex sticky top-0 z-10 bg-white border-b border-pb-border">
                {specialists.map(s => {
                  const initials = `${s.firstName[0]}${s.lastName[0]}`;
                  const count = appointments.filter(a => a.services.some(sv => sv.specialist.id === s.id)).length;
                  return (
              ...
              {/* Body — all columns + single now-line overlay */}
              <div className="relative flex" style={{ height: gridHeight }}>
                ...
                {nowTop !== null && (
                  <div className="absolute left-0 right-0 z-20 pointer-events-none" style={{ top: nowTop }}>
                    ...
                    <div className="h-[1.5px] bg-pb-gold w-full relative">
                      <div className="absolute left-0 top-[-3.5px] w-2 h-2 rounded-full bg-pb-gold" />
                    </div>
                  </div>
                )}
                ...
                {/* Specialist columns */}
                {specialists.map(s => {
                  const specAppts = appointments.filter(a => a.services.some(sv => sv.specialist.id === s.id));
                  return (
                    <div key={s.id} className="w-[185px] flex-shrink-0 relative border-r border-pb-border last:border-r-0">
                      {/* Hour cells with out-of-hours shading */}
                      {hours.map(h => {
                        const offHours = h < DEFAULT_WORK_START || h >= DEFAULT_WORK_END;
                        return (
                          <div key={h}
                            className="border-b border-[#EAE6E0] cursor-pointer"
                            style={{
                              height: CELL_H,
                              background: offHours ? "#EDE9E3" : undefined,
                            }}
                          />
                        );
                      })}
